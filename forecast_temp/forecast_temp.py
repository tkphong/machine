...
def forecast_temperature(temps):
    """
    temps is a list of temperatures, has 384 values
    """
    ...
    for i in range(48):
        ...
        predict_idx_post_midnight[i] = 0.25 * (seasonal_idx_post_midnight[i] + seasonal_idx_post_midnight[48+i] + seasonal_idx_post_midnight[96+i] + seasonal_idx_post_midnight[144+i])

    # Linearly guessing the new average value (pre-00:00 and post-00:00)
    def linear_guess(arr):
        reg = LinearRegression().fit(np.arange(len(arr)).reshape(-1, 1), np.array(arr).reshape(-1, 1))
        return reg.predict([[len(arr)]])[0][0].item()

    # Calculate new average value (pre-00:00 and post-00:00)
    ...
    # Multiply the index to guess the final 30 minutes gap temperature (pre-00:00 and post-00:00)
    predict_temp_30_min_pre_midnight = np.array(predict_idx_pre_midnight) * predict_temp_pre_midnight
    predict_temp_30_min_post_midnight = np.array(predict_idx_post_midnight) * predict_temp_pre_midnight

    # Reduce from 30 minutes gap to 1-hour gap (pre-00:00 and post-00:00)
    ...
    even_idx_temps_post_midnight = predict_temp_30_min_post_midnight[::2]
    odd_idx_temps_post_midnight = predict_temp_30_min_post_midnight[1::2]
    predict_temp_hour_post_midnight = 0.5 * (even_idx_temps_post_midnight + odd_idx_temps_post_midnight)

    # Combine the predicted temperatures of pre-00:00 and post-00:00
    predict_temp_hour = np.concatenate((predict_temp_hour_pre_midnight, predict_temp_hour_post_midnight))
